...
<script type="text/javascript">
	function pageButton(q_name,dateStart,dateEnd,start){
		window.location.href="car_queryData.action?q_name="+q_name+"&dateStart="+dateStart+"&dateEnd="+dateEnd+"&start="+start;
	}
...
.alt td {
	...
	<body>
		<form id="submitForm" name="submitForm" action="car_queryData.action"
			method="post">
				...
				<div class="ui_content">
					<div class="ui_text_indent">
						<div id="box_border">
							...
							<div id="box_center">
								车位区域：
								<input type="text" value="<s:property value="#request.q_location"/>"
									id="q_location" name="q_location" class="ui_input_txt02" style="width: 100px;"/>&nbsp;
								车牌号：
								<input type="text" value="<s:property value="#request.q_carNum"/>"
									id="q_carNum" name="q_carNum" class="ui_input_txt02" style="width: 100px;"/>&nbsp;
								车位状态：
								<select name="q_state" id="q_state" class="ui_select01">
                               		<option value="">--请选择--</option>
                                	<option value="0" <s:if test='#request.q_state=="0"'>selected</s:if>>空位</option>
                                	<option value="1" <s:if test='#request.q_state=="1"'>selected</s:if>>已有</option>
                            	</select>
								开始时间：
				...
				<div class="ui_content">
					<div class="ui_tb">
						<table class="table" cellspacing="0" cellpadding="0" width="100%"
							...
							<s:iterator value="backList" var="car">
								<tr>
									<td>
										<input type="checkbox" name="id"
											value="<s:property value="#car[0]" />" class="acb" />
									</td>
									<td>
										<s:property value="#car[4]" />
									</td>
									<td>
										<s:property value="#car[1]" />
									</td>
									<td>
										<s:property value="#car[2]" />
									</td>
									<td>
										<s:if test='#car[3] == 0'>
											空位
										</s:if>
										<s:elseif test="#car[3] == 1">
											已有
										</s:elseif>
										<s:else>
											<s:property value="#car[3]" />
										</s:else>
									</td>
									<td>
										<s:date name="#car[5]" />
									...
									<td>
										<a class="edit" href="mag_toUpdate.action?id=<s:property value="#car[0]" />
										...
										<a onclick="return firm('<s:property value="#house[1]" />')" href="mag_deleteData.action?id=<s:property value="#car[0]" />
											&q_name=<s:property value="#request.q_name"/>
											&start=<s:property value="#request.(#request.currentPage-1)*7" />
											&dateStart=<s:property value="#request.dateStart"/>
